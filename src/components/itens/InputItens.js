...
function InputItens(randomizer, newContainer){
    ...
    container.appendChild(Button("PLAY", "/src/assets/button/playButton.svg", "linear-gradient(to top, #0ba360 0%, #3cba92 100%)", () => {getList(textArea.value, randomizer, container, newContainer)}))
...
function getList(text, randomizer, containerTextArea, newContainer){
    ...
    list = list.map(element => {
        ...
        return item}).filter(element => element != "")
